...
async function init() {
  ...
  if (params.get('level')) {
    ...
    if (Number.isFinite(w)) worldIdx = THREE.MathUtils.clamp(w, 0, 2);
  ...
  const menu = createMenu({
    onPlay: () => {
      ...
      enterBattle(nxt ? nxt.w : 2, nxt ? nxt.l : 9);
    },
    onSelect: () => showSelect(),
  ...
  const resultModal = createResult({
    ...
    onNext: () => { const b = lastBattleInfo; enterBattle(Math.min(2, b.w + (b.l === 9 ? 1 : 0)), (b.l + 1) % 10); },
  ...
  function showMenu() {
    ...
    menu.root.querySelector('#m-play').textContent =
      nxt ? `▶ 开始冒险（${nxt.w + 1}-${nxt.l + 1}）` : '▶ 已全通关！再战 3-10';
    rig.autoOrbit = true;
  }
  function showSelect() {
    mode = 'select';
