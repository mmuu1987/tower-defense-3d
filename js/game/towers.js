...
export const TOWER_DEFS = {
  arrow: {
    ...
    lvls: [null, { cost: 60, dmg: 24, rate: 1.4, range: 3.7 }, { cost: 115, dmg: 37, rate: 1.6, range: 4.1 }],
  },
  cannon: {
    ...
    lvls: [null, { cost: 100, dmg: 50, splash: 1.9 }, { cost: 170, dmg: 74, splash: 2.2, range: 3.4 }],
  },
  frost: {
    key: 'frost', name: '寒霜塔', hotkey: '3', cost: 90, kind: 'pulse',
    dmg: 10, rate: 0.9, range: 2.9, slow: { pct: 0.48, dur: 2.0 }, targets: 'both', desc: '冰环减速周围敌人',
    lvls: [null, { cost: 80, dmg: 15, slow: { pct: 0.54, dur: 2.3 }, range: 3.1 },
           { cost: 140, dmg: 22, slow: { pct: 0.6, dur: 2.6 }, range: 3.3 }],
  },
  tesla: {
    key: 'tesla', name: '特斯拉塔', hotkey: '4', cost: 130, kind: 'chain',
    dmg: 19, rate: 0.8, range: 3.2, chains: 3, targets: 'both', desc: '闪电链打击多个敌人',
    lvls: [null, { cost: 110, dmg: 28, chains: 4 }, { cost: 190, dmg: 40, chains: 5, range: 3.5 }],
  },
  sniper: {
    ...
    lvls: [null, { cost: 130, dmg: 110, rate: 0.38 }, { cost: 210, dmg: 165, rate: 0.42, range: 7.2 }],
  },
};
...
export function statsFor(key, level) {
  ...
  if (level > 0 && d.lvls[level]) Object.assign(s, d.lvls[level]);
  return s;
}
